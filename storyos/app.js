...
// ── Narrative units (главы / сцены нарративного порядка) ───────
...
/** Find any object by id across all collections. */
...
function renderStatusBadge(status) {
  ...
  return `<span class="detail-status detail-status--${escapeHtml(status)}">${escapeHtml(status)}</span>`;
}

function renderBaseFields(obj) {
  return `
    <div class="detail-header">
      <span class="detail-type">${TYPE_LABELS[obj.type] || obj.type}</span>
      ${renderStatusBadge(obj.status)}
    </div>
    <div class="detail-name">${escapeHtml(obj.name || obj.title || '')}</div>
...
function renderEventDetail(ev) {
  ...
  const locLine = location
    ? `<div class="detail-row"><span class="detail-row-label">Location</span><span>${escapeHtml(location.name)}</span></div>`
    : '';

  const timeLine = ev.storyTime
    ? `<div class="detail-row"><span class="detail-row-label">Story time</span><span class="detail-mono">${escapeHtml(ev.storyTime)}</span></div>`
    : '';

  const orderLine = ev.narrativeOrder != null
    ? `<div class="detail-row"><span class="detail-row-label">Narrative order</span><span class="detail-mono">#${ev.narrativeOrder}</span></div>`
    : '';
...
function renderCharacterDetail(char) {
  ...
  const relList = relations.length
    ? `<div class="detail-section-title">Relations</div>
       <ul class="detail-list">${relations.map(r => `<li>${escapeHtml(r.label)}</li>`).join('')}</ul>`
    : '';

  const evList = events.length
    ? `<div class="detail-section-title">Linked events</div>
       <ul class="detail-list">${events.map(e => `<li>${escapeHtml(e.name)}</li>`).join('')}</ul>`
    : '';
...
function renderLocationDetail(loc) {
  ...
  const evList = events.length
    ? `<div class="detail-section-title">Events here</div>
       <ul class="detail-list">${events.map(e => `<li>${escapeHtml(e.name)}</li>`).join('')}</ul>`
    : '';
...
function renderChapterDetail(unit) {
  ...
  const povLine = povChar
    ? `<div class="detail-row"><span class="detail-row-label">POV</span><span>${escapeHtml(povChar.name)}</span></div>`
    : '';

  const orderLine = unit.narrativeOrder != null
    ? `<div class="detail-row"><span class="detail-row-label">Narrative order</span><span class="detail-mono">#${unit.narrativeOrder}</span></div>`
    : '';
  ...
  return `
    ${renderBaseFields(unit)}
    <div class="detail-meta">${povLine}${orderLine}</div>
    ${evList}
  `;
}

function renderDetail(obj) {
  if (!obj) return '<span class="placeholder-text">Nothing selected</span>';
...
function updateSelectionPanel(obj) {
  ...
  panel.innerHTML = `
    <div class="entity-detail">
      ...
      <div class="entity-detail-meta">${typeLabel}${obj.status && obj.status !== 'n/a' ? ' · ' + escapeHtml(obj.status) : ''}</div>
...
const STUB_REPLIES = [
  ...
  'POV главы 3 — Соколова. Это даёт возможность скрыть информацию от читателя.',
